...
> **[RECONCILIATION NOTE 2026-07-28 — not an amendment to III.7; the ratified text above stands as ratified.]** III.7 defines THREE tiers. The project's operative commit vocabulary (`docs/HANDOFF.md` §5) has since refined these into SIX, and in doing so it **split** III.7's `[externally-verified]` along a line III.7 does not draw: `[externally-grounded]` = "non-Claude engine/data anchors it (design principle)", vs `[externally-verified]` = "external judge on real data confirms the IMPLEMENTATION". III.7's clause above — "an execution engine or judge Claude did NOT author, against input Claude did NOT create ... e.g. the official validator binary" — satisfies the *grounded* half; on its own it does not establish the *verified* half, which additionally requires real data and an implementation claim. **For tagging work in this project, HANDOFF §5 governs; III.7 remains the charter's coarser parent definition.** Recorded because this document and `docs/AUDIT.md` were filed five consecutive times (semantic-layering steps 1–5) as using `[externally-verified]` in a sense predating the split.
